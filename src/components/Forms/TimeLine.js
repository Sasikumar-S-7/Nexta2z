...
import styles from "./requestdemo.module.css"; // Use CSS Module for Next.js styling
...
const Timeline = () => {
  ...
  useEffect(() => {
    // Ensure that we don't reset `activeStep` when the component re-renders
    if (activeStep === 1) return; // Avoid unnecessary resets
    setActiveStep(1);
  }, []);
        ...
        <div className="position-relative d-flex justify-content-between align-items-center">
          ...
          {steps.map((step) => (
            <div key={step.id}>
              ...
              >
                <div
                  className={`rounded-circle shadow ${activeStep === step.id ? "bg-primary text-white" : "bg-light text-dark"}`}
                  style={{
                    width: "50px",
                    height: "50px",
                    lineHeight: "50px",
